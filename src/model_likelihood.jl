...
"""Sample X from U"""
@gen function generateXfromU(U::Any, uxLS::SupportedRBFLengthscale, xScale, xNoise, n::Int64, nX::Int64)
    X = zeros(n, nX)
    for k = 1:nX
        uxCovLog_k = rbfKernelLog(U, U, uxLS[k, :])
        Xcov_k = processCov(uxCovLog_k, xScale, xNoise)
        @assert size(Xcov_k) == (n, n)
        X[:, x_k] = @trace(MappedGenerateX(Xcov_k, fill(n, 1)), :X)
    end
    return X
end
