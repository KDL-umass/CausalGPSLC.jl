...
function conditionalITE(uyLS::Array{Float64, 1}, tyLS::Float64, xyLS::Nothing, 
    ...
    CovWW  = processCov(uyCovLog + tyCovLog, yScale, 0.)
    CovWW  = Symmetric(CovWW)
    CovWWp = Symmetric(CovWW + (yNoise * 1I))
    
    #   K(W, W_*) in the overleaf doc. The cross covariance block is not in general symettric.
    CovWWs = processCov(uyCovLog + tyCovLogS, yScale, 0.)
    
#   K(W_*, W_*) in the overleaf doc.
    CovWsWs = processCov(uyCovLog + tyCovLogSS, yScale, 0.)
    CovWsWs = Symmetric(CovWsWs)
...
function conditionalITE(uyLS::Nothing, tyLS::Float64, xyLS::Array{Float64}, 
    ...
    CovWW  = processCov(xyCovLog + tyCovLog, yScale, 0.)
    CovWW  = Symmetric(CovWW)
    CovWWp = Symmetric(CovWW + (yNoise * 1I))
    
    #   K(W, W_*) in the overleaf doc. The cross covariance block is not in general symettric.
    CovWWs = processCov( xyCovLog + tyCovLogS, yScale, 0.)
    
#   K(W_*, W_*) in the overleaf doc.
    CovWsWs = processCov(xyCovLog + tyCovLogSS, yScale, 0.)
    CovWsWs = Symmetric(CovWsWs)
...
function conditionalITE(uyLS::Nothing, tyLS::Float64, xyLS::Nothing, 
    ...
    CovWW  = processCov(tyCovLog, yScale, 0.)
    CovWW  = Symmetric(CovWW)
    CovWWp = Symmetric(CovWW + (yNoise * 1I))
    
    #   K(W, W_*) in the overleaf doc. The cross covariance block is not in general symettric.
    CovWWs = processCov(tyCovLogS, yScale, 0.)
    
#   K(W_*, W_*) in the overleaf doc.
    CovWsWs = processCov(tyCovLogSS, yScale, 0.)
    CovWsWs = Symmetric(CovWsWs)
